...
```{r CI}
# Calculating beta1 
beta1 <- m$coefficients
beta1
beta1 <- n$coefficients
beta1
...
ci
pi <- predict(m, newdata = data.frame(d$Brain_Size_Species_Mean), interval = "prediction", level = 0.90)  # for a vector of values
...
pi1
pi2 <- predict(n, newdata = log(Brain_Size_Species_Mean = 800), interval = "prediction", level = 0.90)
head(pi2)
```
I would not trust this model to accurately predict this because it is too 
